<?php
    for ($j = 0; $j < 400; $j++) {
        $birth = rand(1960, date("Y") - 15) . '年'. rand(1, 12) . '月'. rand(1, 31) . '日';
        ...
        $address = $address_data[rand(0, 46)];
        $password = '1111';
        $loop = rand(15, 25);
        $email = '';
        for ($i = 0; $i < $loop; $i++)
        ...
        $tel_head = ['050', '070', '080', '090'];
        $tel = $tel_head[rand(0,3)] . rand(0, 9) . rand(0, 9) . rand(0, 9) . rand(0, 9) . rand(0, 9) . rand(0, 9) . rand(0, 9) . rand(0, 9);

        $publish_email = rand(0, 1);
        $publish_tel = rand(0, 1);
        ...
        echo $address . '<br>';
        echo $tel . '<br>';
        echo 'メールアドレス公開：' . $publish_email . '<br>';
        echo '電話番号公開：' . $publish_tel . '<br><br>';

        try {
            $pdo = new PDO("mysql:dbname=test_db;host=run-php-db;", "test", "test");
            $sql = $pdo->prepare("INSERT INTO accounts SET birthday = :birthday, id_name = :id_name, name = :name, email = :email, tel = :tel, gender = :gender, address_1 = :address_1, address_2 = :address_2, password = :password, publish_email = :publish_email, publish_tel = :publish_tel;");
            $sql->execute(array(
                ":birthday" => $birth,
                ":id_name" => $id_name,
                ":name" => $full_name,
